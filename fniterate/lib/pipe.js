...
const mmap = (selector) => (step) => (a, c) => step(a, selector(c));
const mapy = (selector) => (step) => (a, c) => step(a, selector(c));
const mapxy = (selector) => (step) => (a, c) => step(a, selector(c));
const mapxys = (selector) => (step) => (a, c) => step(a, selector(c));
...
export const firstOrDefault = (defaultValue, predicate) => (source) => {
    for (const item of source) {
        if (predicate?.call(source, item) ?? true) {
            return item;
        }
